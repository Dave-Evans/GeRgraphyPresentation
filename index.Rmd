...
## Vector Data Intro {.columns-2}

* How data is represented
Vector data is represented by coordinate pairs,
...
Geometry is associated with other data, _attribute data_
Conceptualize as a row in a table
![think like a table](img/table_concept.png)


* What vector data is used for
<div class="notes">
Geometry is just another field in the table, mention multipart features
</div>
...
## Coordinate Reference Systems | A (Very) Brief Break
A coordinate reference system (CRS) defines the surface of the world. 
There are many and if they don't match, errors and issues can arise
![different map projections](img/map_projections.png)

<div class="notes">Coordinate reference systems are ways of referencing X and Y (longitude and latitdue) to specific points on the Earth. When they don't match, identical points won't lay on top of one another.</div>

## Coordinate Reference Systems | A (Very) Brief Break {.columns-2}
...
How do two shapes relate?
Intersects for if the geometries have at least one point in common or no points in common (disjoint)
...
```
<div class="notes">
Heard these referred to as "Binary Predicates"
Now that they have the same projection we can look at how they relate. We'll assume that we are interested in what the soil texture is at the location of the well sites. But first an intro into geometric relationships
</div>

## Vector Data Geometry Relationships {.columns-2 .smaller}
List other relational operators.
Dave, also demo clipping or unioning or...some geometry manipulation
Introduce using rgeos examples
...
<div class="notes">
In practice its a bit more complicated, our question is which soil polygons have wells in them?
</div>
...
<div class="notes">
A matrix denoting when there is an intersections
</div>
...
```
<div class="notes">
You'll have to pardon me, I don't yet know magrittr piping...
</div>
...
```{r echo=F, message=F, fig.width=4, fig.height=5}
plot(wards, main="Starting point")
```
...
```{r eval=F, message=F, fig.width=4, fig.height=5}
par(mfrow(3,1))
...
<div class="notes">
Think about the shape of the data and what classification scheme we would like to use.
</div>
...
## Vector Data Modeling {.columns-2 .smaller}
wards_centroids = gCentroid(wards, byid=T)
wards_centroids = SpatialPointsDataFrame(
    gCentroid(wards, byid=T), 
    over(wards_centroids, wards)
