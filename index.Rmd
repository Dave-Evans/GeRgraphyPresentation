...
```{r}
sl2 = SpatialLines(list(S2))#,S2
```

## Vector Data Intro {.smaller}

Geometry is associated with other data, known as _attribute data_.
Conceptualize as a row in a table

![think like a table](img/table_concept.png)

Each row is one observation, referred to as a _feature_. This becomes important later, are we referring to individual features or to the entire geometry.
...
## Vector Data {.smaller}
Another kind of relationship functions return the geometry (versus returning booleans)
...
## Vector Data {.smaller}
We use the `gIntersection` function, this returns that area to us
`xyinter = gIntersection(x, y)`. Other functions like `gDifference`, `gUnion`, or `gSymdifference` also return geometry.
...
Grab data within a certain range of the middle? 
Note how this returns geometry
```{r tidy=T, eval=F}
